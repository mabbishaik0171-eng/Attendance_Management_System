...
if(isset($_POST['submit'])){
  ...
  $stmt = $conn->prepare("
      INSERT INTO faculty (username, fname, gender, fmail, fpno, password)
      VALUES (?, ?, ?, ?, ?, ?)
  ");

  $stmt->bind_param("ssssss", $username, $fname, $gender, $fmail, $fpno, $password);

  if($stmt->execute()){
      // get last insert id
      $last_faculty_id = $conn->insert_id;

      // show subject assignment dialog
      echo "<script>
              localStorage.setItem('new_faculty_id', '$last_faculty_id');
              window.location.href='assign_subject.php';
            </script>";
      exit();
  }
  else{
